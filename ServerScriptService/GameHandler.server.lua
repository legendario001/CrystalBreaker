...
Events.ThrowBall.OnServerEvent:Connect(function(player, targetPos)
	...
	-- Efecto visual del golpe (pelota que impacta)
	...
	if char then
		...
		if root then
			task.spawn(function()
				...
				ball.Position = root.Position + Vector3.new(0, 2, 0)
				ball.Parent = Workspace

				local start = ball.Position
				local finish = nearest.Position
				for t = 0, 1, 0.15 do
					ball.Position = start:Lerp(finish, t) + Vector3.new(0, math.sin(t * 3.14) * 5, 0)
					task.wait(0.02)
				end
				ball:Destroy()
			end)
