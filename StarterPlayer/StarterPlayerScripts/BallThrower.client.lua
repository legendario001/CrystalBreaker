...
local BuyBlockEvent = ReplicatedStorage:WaitForChild("BuyBlock", 15)
local UpdateInventoryEvent = ReplicatedStorage:WaitForChild("UpdateInventory", 15)
...
-- Money panel
-- ============================================
-- CONTADOR DE DINERO (centro arriba) - OPTIMIZADO
...
moneyStroke.Thickness = 2 moneyStroke.Transparency = 0.2 moneyStroke.Parent = moneyPanel
...
local MONEY_SUFFIXES = {
        ...
        {1e6,  "Millón",    "Millones",    Color3.fromRGB(129, 199, 132),  Color3.fromRGB(180, 240, 180)}, -- verde brillante
        {1e3,  "Mil",       "Mil",         Color3.fromRGB(170, 220, 170),  Color3.fromRGB(200, 240, 200)}, -- verde claro
}

-- Color por defecto cuando no hay sufijo (numeros chicos < 1000)
local MONEY_COLOR_DEFAULT = Color3.fromRGB(255, 255, 255)
local MONEY_COLOR_BRIGHT_DEFAULT = Color3.fromRGB(240, 240, 240)
...
local function formatMoneyClient(amount)
        ...
        -- Menos de 1000: numero puro
        return "$" .. tostring(math.floor(amount))
end

-- Funcion para obtener el color segun el rango (para flash dorado en altos montos)
local function getMoneyColor(amount)
        amount = amount or 0
        for _, tier in ipairs(MONEY_SUFFIXES) do
                if amount >= tier[1] then
                        return tier[4], tier[5] -- color base, color bright
                end
        end
...
local function applyMoneyUpdate(amount, forceInstant)
        ...
        -- Reset flag por frame
        popPlayedThisFrame = false
...
MoneyUpdateEvent.OnClientEvent:Connect(function(amount)
        ...
        -- Si es la primera vez (cantidad muy diferente) o paso el throttle, aplicar
        if lastMoneyUpdate == 0 or (now - lastMoneyUpdate) >= THROTTLE_INTERVAL then
                lastMoneyUpdate = now
                applyMoneyUpdate(amount, false)
                pendingMoneyAmount = nil
        else
                -- Guardar para aplicar despues
                pendingMoneyAmount = amount
        end
end)

-- Loop para aplicar el pending amount cuando se libera el throttle
-- (asegura que el ultimo valor siempre se muestre)
task.spawn(function()
        while true do
                task.wait(THROTTLE_INTERVAL)
                if pendingMoneyAmount ~= nil then
                        local now = tick()
                        if (now - lastMoneyUpdate) >= THROTTLE_INTERVAL then
                                lastMoneyUpdate = now
                                local amt = pendingMoneyAmount
                                pendingMoneyAmount = nil
                                applyMoneyUpdate(amt, false)
                        end
                end
        end
end)

-- Aplicar estado inicial (cuando carga el script)
moneyLabel.Text = "$0"
...
-- ============================================
-- TweenService ya fue declarado arriba (seccion MONEY UPDATE)
...
-- ============================================
-- SISTEMA DE DINERO OFFLINE (ModuleScript OfflineEarningsSystem)
-- ============================================
local OfflineEarningsSystem = require(script.Parent:WaitForChild("OfflineEarningsSystem", 10))
if OfflineEarningsSystem then
        OfflineEarningsSystem.init({
                player = player,
                screenGui = screenGui,
                ReplicatedStorage = ReplicatedStorage,
        })
        print("Sistema de dinero offline cargado!")
else
        warn("[OfflineEarnings] No se pudo cargar OfflineEarningsSystem ModuleScript")
end

-- ============================================
-- SISTEMA DE CONSTRUCCION (modo build)
-- ============================================
-- Extraido a ModuleScript BuildSystem.lua para evitar superar el limite
