...
-- ============================================
-- GLOBAL ERROR HANDLER - Captura cualquier error no manejado
-- Evita que el servidor se pause/crashee
-- ============================================
local function safeErrorHandler(errorMessage)
        warn("[CrystalBreaker SAFE] " .. tostring(errorMessage))
end
...
-- FIX: pcall INDIVIDUAL por cada personaje para maxima estabilidad
-- FIX: Verificar instancias con pcall individual para evitar crashes del engine
-- ============================================
task.spawn(function()
        while true do
                task.wait(2)
                -- Recolectar lista de jugadores activos primero (evitar iterar tabla que cambia)
                local activePlayers = {}
                ...
                for _, playerEntry in ipairs(activePlayers) do
                        ...
                        for _, entry in ipairs(charList) do
                                ...
                                if not charOk then
                                        -- Error en un personaje especifico - no crashea todo el timer
                                        warn("[MoneyTimer] Error en personaje: " .. tostring(charErr))
                                end
                        end
                end
        end
