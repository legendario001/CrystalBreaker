...
local MAX_REBIRTH = 5
-- Sistema de multiplicador ACUMULABLE:
-- Renacer 1 da x1, renacer 2 da x2 (total x3), renacer 3 da x3 (total x6), etc.
-- Multiplicador total = suma de 1 hasta rebirthLevel = rebirthLevel * (rebirthLevel + 1) / 2
-- Ejemplos: rebirthLevel=1 -> x1, =2 -> x3, =3 -> x6, =4 -> x10, =5 -> x15
...
-- Devuelve el multiplicador total de renacimiento (acumulable)
-- rebirthLevel=1 -> x1, =2 -> x3, =3 -> x6, =4 -> x10, =5 -> x15
function RebirthManager.getRebirthMultiplier(rebirthLevel)
        local level = rebirthLevel or 0
        if level <= 0 then return 1 end
        -- Suma de 1 hasta level = level * (level + 1) / 2
        return level * (level + 1) / 2
end

-- Devuelve el multiplicador que se GANA al renacer al siguiente nivel
-- rebirthLevel=0 -> ganaria x1, =1 -> ganaria x2, =2 -> ganaria x3, etc.
function RebirthManager.getNextRebirthMultiplier(rebirthLevel)
        local level = rebirthLevel or 0
        if level >= MAX_REBIRTH then return 0 end
        return level + 1  -- el multiplicador que gana = nuevo nivel
end

-- Devuelve el multiplicador total que tendria si renace ahora
function RebirthManager.getNextTotalRebirthMultiplier(rebirthLevel)
        local level = rebirthLevel or 0
        if level >= MAX_REBIRTH then
                return RebirthManager.getRebirthMultiplier(level)
        end
        return RebirthManager.getRebirthMultiplier(level + 1)
end

-- COMPATIBILIDAD: getTotalBonusPercent sigue existiendo para no romper codigo viejo
-- Pero ahora devuelve solo el boost (porcentaje), el rebirth es multiplicador separado
function RebirthManager.getTotalBonusPercent(boostLevel, rebirthLevel)
        local boost = (boostLevel or 0) * 20
        return boost  -- solo boost en porcentaje, rebirth es multiplicador aparte
end
...
function RebirthManager.performRebirth(player, callbacks)
        ...
        local rebirthMult = RebirthManager.getRebirthMultiplier(newLevel)
        if callbacks.notifyClient then
                callbacks.notifyClient(player, newLevel, rebirthMult)
        end

        -- 6. Respawnear al jugador (para que empiece limpio)
        player:LoadCharacter()

        print("[Rebirth] " .. player.Name .. " renacio al nivel " .. newLevel .. " (multiplicador total: x" .. rebirthMult .. ")")
